...
Core insight: wavelength-division multiplexing means we can process
all 64 wavelength channels in parallel on the GPU. Each switch processes
its entire spectrum simultaneously.

UPDATE (Phase-aware training bridge):
- Adds Trainable connectivity stored as (real, imag) nn.Parameters
- Adds phase-preserving activation that avoids torch.angle discontinuities
- Keeps the original demo/benchmark functionality intact
"""
...
class PhotonicNetworkGPU(nn.Module):
    ...
    Network state:
        [batch, n_switches, n_wavelengths, n_modes] complex
    ...
    This maps naturally to GPU parallelism:
        ...
        - wavelength dimension: WDM channels
    ...
    def __init__(self, config: GPUConfig):
        ...
        # Assign switch types in structured blocks (regions)
        ...
        # Connectivity matrix: [n_switches, n_switches] complex (NOT trainable here)
        ...
        # Optional per-switch phase shift (useful for training / path length modeling)
        self.switch_phase = nn.Parameter(torch.zeros(config.n_switches, device=self.device))
    ...
    def _init_connectivity(self) -> torch.Tensor:
        ...
        Principles:
        - Same-type switches: dense local connectivity
        - Cross-type switches: sparse integration pathways
        - Weak hierarchy within types: feedforward + weaker feedback
        """
        n = self.config.n_switches
        ...
        SAME_TYPE_DENSITY = 0.4
        SAME_TYPE_STRENGTH = 0.3
        CROSS_TYPE_DENSITY = 0.05
        ...
        return conn.to(self.config.dtype)
    ...
    def inject(self, switch_idx: int, signal: torch.Tensor):
        ...
        """
        if self.field is None or self.output is None:
            self.reset(signal.shape[0])

        if signal.dim() == 2:
            signal = signal.unsqueeze(1).expand(-1, self.config.n_wavelengths, -1)

        if signal.dtype.is_floating_point:
            signal = signal.to(torch.float32)
            signal = torch.complex(signal, torch.zeros_like(signal))

        signal = signal.to(self.config.dtype).to(self.device)
        self.field[:, switch_idx, :, :] = self.field[:, switch_idx, :, :] + signal

    def _apply_switch_phase(self, x: torch.Tensor) -> torch.Tensor:
        """
        Apply per-switch phase shift: multiply by exp(i * phase_i)
        x: [batch, n_switches, n_wavelengths, n_modes] complex
        """
        phase = self.switch_phase.unsqueeze(0).unsqueeze(-1).unsqueeze(-1)  # [1, sw, 1, 1]
        return x * torch.exp(1j * phase)

    def _phase_preserving_activation(self, x: torch.Tensor) -> torch.Tensor:
        """
        Phase-preserving nonlinearity WITHOUT torch.angle():
            new_mag = sat * tanh((mag * gain + bias)/sat)
            out = x * (new_mag / (mag + eps))

        This keeps phase continuous (except at exact zero) and avoids angle() discontinuities.
        """
        cfg = self.config
        eps = 1e-8
        ...
        sat = cfg.saturation
        new_mag = sat * torch.tanh((mag * gain + bias) / sat)

        scale = new_mag / (mag + eps)
        return x * scale
    ...
    def step(self) -> torch.Tensor:
        ...
        cfg = self.config
        if self.field is None or self.output is None:
            self.reset(cfg.batch_size)

        # 1) GATHER
        out_t = self.output.permute(0, 2, 3, 1)  # [batch, λ, m, sw]
        gathered = torch.matmul(out_t, self.connectivity)  # complex matmul
        gathered = gathered.permute(0, 3, 1, 2)  # [batch, sw, λ, m]
        ...
        for _ in range(cfg.integration_steps):
            ...
            if cfg.dispersion_strength > 0:
                phase = self.dispersion_shape * cfg.dispersion_strength  # [λ]
                self.field = self.field * torch.exp(1j * phase).unsqueeze(0).unsqueeze(0).unsqueeze(-1)

            self.field = self.field + gathered * 0.3

        # 2.5) switch-specific phase shift (models path length)
        self.field = self._apply_switch_phase(self.field)

        # 3) INTERPRET wavelengths
        weighted = self.field * self.wavelength_weights.unsqueeze(0).unsqueeze(-1)

        # 4) ACTIVATE (phase-preserving, stable)
        self.output = self._phase_preserving_activation(weighted)
...
# ══════════════════════════════════════════════════════════════════════════════
# TRAINABLE BRIDGE: Connectivity as (real, imag) Parameters
# ══════════════════════════════════════════════════════════════════════════════

class TrainablePhotonicNetworkGPU(PhotonicNetworkGPU):
    """
    A drop-in PhotonicNetworkGPU variant designed for training.

    Key differences:
    - Connectivity is trainable and stored as two real Parameters:
        self.conn_real, self.conn_imag
      so we can avoid fragile complex autograd failure modes and keep phase stable.

    - Activation is already phase-preserving and avoids angle().
    """

    def __init__(self, config: GPUConfig, init_connectivity: Optional[torch.Tensor] = None,
                 conn_scale: float = 0.30, init_phase_std: float = 0.10):
        super().__init__(config)

        # Replace fixed connectivity with trainable real/imag parameters.
        # If an initial connectivity is provided, we use its real part as a magnitude baseline.
        if init_connectivity is None:
            init_connectivity = self.connectivity.detach()

        # Magnitude baseline: scaled absolute value of initial real weights
        base_mag = init_connectivity.real.abs() * conn_scale  # [sw, sw] float

        # Small random phase
        phase = torch.randn_like(base_mag) * init_phase_std

        conn_r = base_mag * torch.cos(phase)
        conn_i = base_mag * torch.sin(phase)

        self.conn_real = nn.Parameter(conn_r.to(torch.float32))
        self.conn_imag = nn.Parameter(conn_i.to(torch.float32))

        # Keep a non-trainable mask if you want to preserve sparsity structure
        # (optional but useful to prevent "densifying" the graph during training)
        self.register_buffer("conn_mask", (init_connectivity.real != 0).to(torch.float32))

        # Disable the old fixed tensor (avoid accidental use)
        self.connectivity = None

    def get_connectivity(self) -> torch.Tensor:
        """
        Return complex connectivity with sparsity mask applied.
        """
        r = self.conn_real * self.conn_mask
        i = self.conn_imag * self.conn_mask
        return torch.complex(r, i).to(self.config.dtype)
    ...
    def step(self) -> torch.Tensor:
        ...
        # 1) GATHER
        out_t = self.output.permute(0, 2, 3, 1)  # [batch, λ, m, sw]
        gathered = torch.matmul(out_t, conn)
        gathered = gathered.permute(0, 3, 1, 2)  # [batch, sw, λ, m]
        ...
        for _ in range(cfg.integration_steps):
            ...
            self.field = self.field + gathered * 0.30

        # 2.5) switch-specific phase shift
        self.field = self._apply_switch_phase(self.field)

        # 3) INTERPRET
        weighted = self.field * self.wavelength_weights.unsqueeze(0).unsqueeze(-1)

        # 4) ACTIVATE (stable, phase-preserving)
        self.output = self._phase_preserving_activation(weighted)
        return self.output
...
class HolographicMemoryBank(nn.Module):
    ...
    def recall(self, cue: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        for i in range(self.n_stored):
            ...
            decoded_content = decoded.sum(dim=0)
        ...
        confidence, best_idx = scores.max(dim=-1)
        best_keys = self.keys[best_idx]
        best_holograms = self.hologram[best_idx]
...
def demo_wavelength_semantics():
    ...
    signal[0:16, :] = torch.exp(-x**2 / 0.1)
    signal[16:32, :] = torch.sin(8 * math.pi * x)
    signal[32:48, :] = torch.ones_like(x) * 0.5
    signal[48:64, :] = torch.cos(2 * math.pi * x)
...
def demo_pattern_recognition():
    ...
    for i, name in enumerate(pattern_list):
        rec = torch.abs(recalled[i]).sum(dim=0)
        rec = rec / (rec.max() + 1e-10)
...
def demo_network_dynamics():
    ...
    for t in range(50):
        ...
        output_energy = (torch.abs(net.output) ** 2).sum(dim=(0, 2, 3))
...
def demo_custom_topology():
    ...
    builder.add_region("V1", SwitchType.VISUAL, count=12)
    builder.add_region("V2", SwitchType.VISUAL, count=10)
    builder.add_region("V4", SwitchType.VISUAL, count=8)
    builder.add_region("IT", SwitchType.VISUAL, count=6)

    builder.add_region("HC", SwitchType.MEMORY, count=8)
    builder.add_region("ATT", SwitchType.ROUTING, count=6)
    ...
    print("\n  Signal propagates: V1 → V2 → V4 → IT → HC (with feedback from ATT)")
